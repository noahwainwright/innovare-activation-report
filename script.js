...
function renderActivationPipeline(accounts) {
  ...
  el.innerHTML = steps.map((step, i) => `
    <div class="pipeline-step">
      ...
      ${i === 0 && DATA.accountScorecard ? `
        <div class="pipeline-scorecard-caption">
          <span class="pipeline-scorecard-text">${DATA.accountScorecard.totalAccounts} cohort accounts visited · ${DATA.accountScorecard.metrics.find(m => m.key === 'dashboard_visit')?.totalUsers || 0} unique users · ${DATA.accountScorecard.windowWeeks}-week window</span>
          <span class="pipeline-scorecard-arrow">→ 0 generated CIWPs</span>
        </div>` : ''}
